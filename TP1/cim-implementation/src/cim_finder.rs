...
impl NeighborFinder<ParticlesData, ID> for CimNeighborFinder {
    fn find_neighbors(particles: &ParticlesData) -> NeighborMap<ID> {
        ...
        let get_cells_to_check = |cell_index: Vector2<usize>| {
            [vec2(0, 0), vec2(1, 0), vec2(1, 1), vec2(0, 1), vec2(-1, 1)].map(|v| {
                (v + cell_index.cast().unwrap())
                    .map(|v| v.rem_euclid(&(particles.grid_size as i32)) as usize)
            })
        };
            ...
            for cell in get_cells_to_check(cell_index) {
                for other in &cells[(cell.y, cell.x)] {
                    if other.id > particle.id {
                        if particle.is_within_distance_of(other, particles.interaction_radius) {
