    ...
    def decorator(func):
        def wrapper(input_generator):
            ...
            with ThreadPool() as pool:
                for i, run in enumerate(pool.imap(func, (input_generator() for _ in range(times)))):
                    run['run'] = i
...
def run_simulation(input_data: str):
    ...
    analyzer_process = subprocess.Popen(["make", "-s", "run-raw", "BIN=analyze", "USE_DOCKER=FALSE",
